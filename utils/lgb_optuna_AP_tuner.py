...
>>> Modified based on lgb_optuna_tuner.py, which has "Metric Mismatch: LightGBM's internal
 metric="auc" optimizes ROC AUC, not AP, so there's a disconnect—-Optuna maximizes sklearn 
 AP post-training, but the booster itself prioritizes ranking over precision-recall balance."
"""
...
def run_lgb_optuna(
    ...
    *,
    ...
    def _objective(trial: optuna.Trial) -> float:
        params = {
            "objective": "binary",
            "metric": "average_precision",  # Prioritize AP
            "verbosity": -1,
            "boosting_type": "gbdt",
